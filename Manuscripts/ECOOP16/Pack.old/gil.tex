...
\begin{figure}[hbt]
  \caption[Type encoding of partial binary function]{\label{Figure:simple-binary-a}    Type encoding of partial binary function~$f:R⨉S↛Γ$,
    where~$R=❴r₁,r₂❵$,~$S=❴s₁,s₂❵$, and~$f$
    is specified by~$f(r₁,s₁)=γ₁$,~$f(r₁,s₂)=γ₂$,~$f(r₂,s₁)=γ₁$, and~$f(r₂, s₂)=⊥$
...
\end{figure}
\begin{figure}[hbt]
  \caption[Use cases of partial binary function]{\label{Figure:simple-binary-b}    Use cases of partial binary defined in \cref{Figure:simple-binary-a} 
  }
...
(\cref{Figure:unary-function-b}), i.e., either~\cc{$γ$1} or~\cc{$γ$2}.
