Ever after their inception~\cite{inception:fowler? not sure} \emph{fluent APIs}
  increasingly gain popularity~\cite{poplularity:list} and research
  interest~\cite{research:interest:list}.
Inspired by the theory of formal languages~\cite{some:text:book} and pushdown automata,
...
Practically speaking,  the poof shows that the (potential)
  ...
  is the same as ANTLR~\cite{give:proper:citation:not:link}:
...
Again, we give reason why this might be possible in some cases,
  it is highly unlikely that fluent APIs can be made for all context free languages.
