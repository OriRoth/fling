...
An JLR item is the tuple~$⟨i, lr ⟩∈Ξ∪❴S'❵$
  where~$i$ is a classic LR(1) item
  and~$lr$ is the \emph{last reduction} of the item, i.e.,
  if~$ lr = A~$, then when we reduce on this (instance of the)
  rule, the last reduction on the consecutive reduction
  list will be of type~$A \rightarrowα$.
We denote this triplet as~$[A \rightarrowα\cdotβ, b , B]$
  where~$A \rightarrowα\cdotβ, b$ is the
  LR(1) notation, and~$B$ is the left-hand side of the last reduction.
...
\begin{algorithm}
  \caption{\label{Algorithm:closure}
    \textsf{Closure} (\textsf{K})}
  \begin{algorithmic}
    \REQUIRE \textsf {K} is a set of kernel items
    \ENSURE $\textsf{Clo} = \textsf{K}∪\textsf{NK}$
    \STATE $\textsf{Clo} \, := \, \textsf{K}$
    \REPEAT
      \FOR{each item~$\left[ A \rightarrowα·Bβ\, , \, a \, , \, C \right]∈\textsf{Clo}$}
  \FOR{each production~$B \rightarrowγ∈G'$}
    \FOR{each terminal~$t∈\textsf{First}(β)$}
      \STATE~$\textsf{Clo} := \textsf{Clo}∪❴ \left[ B→·γ, t , B \right] ❵$
    \ENDFOR
    \IF{$\textsf{Nullable(\beta)}$}
      \STATE~$\textsf{Clo} := \textsf{Clo}∪❴ \left[ B→·γ, a , C \right] ❵$
    \ENDIF
  \ENDFOR
      \ENDFOR
    \UNTIL{no more changes in \textsf{C}}
  \end{algorithmic}
\end{algorithm}
