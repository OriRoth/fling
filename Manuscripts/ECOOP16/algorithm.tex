...
merges $Q^{X}_{Y},Q^{B}_{X}$ into an extended instantiation of $Q_{XY}^B$
...
\begin{enumerate}
\item If $\left(\left(A,i-1,a\right)\in Q^{B}_{X}\right)\wedge\left(\left(A,i-2,a\right)\notin Q^{B}\right)$ //Inherited TA or Synthesized from $Q^B$
...
\begin{enumerate}
\item $Q_{XY}^{B}\left[\left(A,i,a\right)\right]:=Q^{X}_{Y}\left[\left(A,i-1,a\right)\right]$
\end{enumerate}
\end{enumerate}
\end{enumerate}
\end{description}
