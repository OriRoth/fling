...
\begin{algorithm}[p]
  ...
  \begin{algorithmic}[1]
    ...
    \LET{$i$}{$\function dequeue(\Queue)$} \COMMENT{Next JLR item to process}
      ...
      \ENDIF
      \FOR[Add to closure whatever~$Bβ$ may derive]{$B→γ∈G'$,$γ≠ε$}
      \IF[Add to closure item representing~$B\overset*⇒ε$]{$\function Nullable(β)$}
          \STATE{$\function enqueue(\Queue,[B→·γ,a,ℓ])$}
        \ENDIF
        \FOR[Add to closure item representing derivation~$B\overset*⇒t⋯$]{$t∈\function First(β)$}
      ...
      \IF[Add to closure item representing derivation~$B\overset*⇒ε$]{$\function Nullable(B)$}
        \STATE{$\function enqueue(\Queue,[A→αB·β,a,ℓ])$}
      \ENDIF
    \ENDWHILE
    \RETURN{$J$}
  \end{algorithmic}
