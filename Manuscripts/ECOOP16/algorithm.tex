...
It was fabricated with great inspiration from the classical
  LR(1) recognizer, with changes regarding the automaton's
...
For the convenience of the reader, the algorithms are depicted
  in~\cref{Algorithm:Closure},~\cref{Algorithm:Goto} and~\cref{Algorithm:Items}
  accordingly.
\renewcommand{\algorithmicrequire}{\textbf{Input}}
\renewcommand{\algorithmicensure}{\textbf{Output}}
\newcommand\Address{\ensuremath{\textrm{Addr}}}
...
\begin{algorithm}[p]
  ...
  \begin{algorithmic}
    ...
    \STATE{$\Stack←∅$ \COMMENT{A stack used for intermediate computations}}
    \STATE{$J←I$ \COMMENT{Initial approximation of returned value}}
    \FOR{$i∈I$} \STATE{\function push(S, i)} \ENDFOR
    \WHILE{$\Stack ≠ ∅$}
    \LET{$i$}{$\function pop(\Stack)$} \COMMENT{Next item to process}
    \STATE{$J←J ∪ ❴i❵$}
  \STATE{\textbf{Let}~$[A→α·Bβ, a, \Address]←i$}
    \LET{$\Address'$}{\function fresh()}
      \FOR{each production~$B→γ ∈ G'$}
        \IF{$\function NULLABLE(β)$}
        \STATE{$\function push(S, [ B→·γ, a , \Address] )$}
        \ENDIF
        \FOR{$t ∈ \function FIRST(β)$}
        \STATE{$\function push(S, [ B→·γ, t , \Address'] )$}
        \ENDFOR
      \ENDFOR
    \ENDWHILE
    \RETURN{$J$}
...
\begin{algorithm}[p]
  ...
  \begin{algorithmic}
    ...
    \FOR{$i∈I$}
      \LET{$[A→α·Xβ,a,\Address]$}{$i$}\COMMENT{Break item $i$ into components}
      \LET{$i'$}{$[A→αX·β,a,\Address]$}\COMMENT{Advance the dot}
      \STATE{$J←J ∪ ❴i'❵~$}
    \ENDFOR
...
\begin{Lemma}
  ...
  The set of states generated by \cref{Algorithm:Items} is the same as that …
\end{Lemma}
