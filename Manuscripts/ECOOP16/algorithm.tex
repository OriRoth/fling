...
\begin{description}
  ...
  \item[Specification of state transition] with the aid of two tables:
        \begin{description}
          ...
          function~$η:Q⨉Σ↛ ❴ \textsf{Shift}(q') \,|\, q'∈Q❵ ∪ ❴\textsf{Reduce}(r) \,| \, r∈G❵$.
        \end{description}
\end{description}
The parser begins by pushing~$q₀$ (the initial stack()),
and then repetitively execute the following:
Let~$q∈Q$ be the state at the top of the stack.
If~$q=qₙ$ (the accepting state), then the parser accepts and stops.
Let~$σ∈Σ$ be the next input symobl,
If~$η(q,σ)=⊥$, the parser fails.
If~$η(q',σ) = \textsf{Shift}(q')$, the automaton pushes state~$q'$ onto the stack.
If however,~$a(q,σ) = \textsf{Reduce}(r)$,
where~$r=ξ→β$,~$β∈\left(Σ∪Ξ\right)^*$,
the automaton pops~$|β|$ stack symbols.
Let~$q'$ be state at the top of the stack after these pops. 
Then, pop~$q'$, and push instead state $q''$, defined by the transition function~$q”=δ(q',ξ)$.
