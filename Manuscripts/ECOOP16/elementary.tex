...
Items stored in this stack are members of set~$Γ$.
With type encoding this are precisely classes~\cc{$γ$}1
  and \cc{$γ$2} defined in \cref{Figure:unary-function}.
The type of variable~\cc{\_1}⏎
...
encodes in it the sequence of pushing the value~$γ₁$ into an
  ...
  The non-generic type~\cc{E} (a more full name is \cc{Stack.E}†{%
    The~\cc{E} abbreviation is made possible
      thanks to an appropriate \kk{import} statement.
      For brevity, all \kk{import} statements are omitted.}
  represents the empty stack.
The generic type~\cc{P} takes two arguments, the \emph{top} of the stack,
    which must be a subtype of \cc{$Γ$}, and the \emph{rest} of the stack.
...
\begin{figure}[htb]
  \caption{\label{Figure:stack:encoding} Type encoding of a stack data structure}
    \begin{tabular}{cc}
      ...
      &
      \hspace{1ex}
      \parbox[c]{0.86\linewidth}{\javaInput{stack.listing}}
⏎
...
\kk{class}~\cc{Stack.E} represents the empty stack,~\kk{class}~\cc{Stack.¤} represents the error state,
...
\cc{Stack<Tail>} has the two basic methods~\cc{pop()} and~\cc{top()} that return objects of types~\cc{Tail} and~\cc{Γ'}
