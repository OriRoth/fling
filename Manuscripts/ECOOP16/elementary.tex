...
\begin{figure}[hbt]
  \caption{Type encoding of the unary function~$g: T→T$,
    defined by~$T=❴t₁,t₂❵$,~$g(t₁)=t₂$,~$g(t₂)=t₁$.}
...
abstract class T {¢¢ // Set ¢$T$¢
  ...
  static final class t1 extends T {¢¢ // Value ¢$t₁∈T$¢
    @Override t2 g() {¢¢ return null; }
  }
  static final class t2 extends T {¢¢ // Value ¢$t₂∈T$¢
    @Override t1 g() {¢¢ return null; }
  }
}
    \end{Code}
\end{minipage}%
...
t2 x = new t1().g(); // ¢✓¢ 
t1 y = new t2().g(); // ¢✓¢ 
t1 x = new t1().g(); // ¢✗¢ compile time error
t2 y = new t2().g(); // ¢✗¢ compile time error
\end{code}
\end{minipage}
\end{figure}
