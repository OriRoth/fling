...
A \emph{jump-stack} is a stack data structure whose elements are drawn from a finite set~$Γ$,
    ...
    which means
  ‟repetitively pop elements from the stack up to and including the first occurrence of~$γ$”.
Let~$k=|Γ|$.
...
The return type of \cc{$γ$1()} and \cc{$γ$2()} can be written more succinctly.
Consider, e.g.,~$τ=\cc{P<γ1, Me, Me, J\_γ2>}$, the return type of \cc{$γ$1()}.
Type~$τ$ is a generic instantiation of \cc{J.P}, to say that after pushing~\cc{$γ$1},
  we obtain a compound jump-stack, whose top element is \cc{$γ$1}, while the rest of the stack is
  the curent type.
...
Use cases for the encoded jump stack data structure are presented in \cref{Figure:jump-stack-example}.
