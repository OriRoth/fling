...
\begin{quote}
  \javaInput[,minipage,width=45ex]{prefix-proof.many.listing}
\end{quote}
...
\begin{description}
 \item[\textsf{reject}] Since we add the methods ad-hock to each type, the reject entry means
   that the corresponding type, \emph{won't} have a~\cc{\$()} method, i.e., type~\cc{C$γ$1}
 ...
 \item[$⊥$] We encounter~$⊥$ on the transition function when some input character~\cc{$σ$}
   ...
   \emph{must not} have a method for~\cc{$σ$}, this way, invoking the methods will cause type error.
   In \cref{Table:A}~$⊥$ occurs when the top of the stack is~$γ{}2$ and the input character is~$σ{}1$,
   thus, no method \cc{$σ$1} is introduced in type~\cc{C$γ{}$2}.
\end{description}
