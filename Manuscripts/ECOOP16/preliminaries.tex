...
as in the \nth{3} to \nth{6} line of the \Java function
in \cref{Figure:chaining}
...
Method chaining means that the same object, \cc{sb} of type \cc{StringBuilder} in the figure 
   is the receiver of a chain of methods.
There is syntactic sugar for method chaining in languages, 
  e.g., \Smalltalk where it takes the name ``\emph{cascading}''. 
In \Java, the method chaining of \cref{Figure:chaining} is 
  made possible by making each overloaded variant of 
  of \cc{append} return the receiver, i.e., \kk{this}.
To appreciate the convention, compare with the 
  non-chaining version in \cref{Figure:non-chaining}.
  
\begin{wrapfigure}[10]{r}{44ex}
  \caption{\label{Figure:non-chaining}%
    A non-chaining version of \cref{Figure:chaining}
  }
  \begin{lcode}[minipage,width=\linewidth]{Java}
String time(int hours, int minutes, int seconds) {¢¢
  StringBuilder sb = new StringBuilder();
