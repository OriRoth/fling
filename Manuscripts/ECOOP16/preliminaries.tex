...
A CFG~$G$ has three components:~$Ξ$ a set of \emph{variables} (also called nonterminals), % It is worth adding the fourth element- alphabet or at least add a footnote on it.
...
\begin{Definition}[Deterministinc Pushdown Automaton]
  ...
  A \emph{deterministic pushdown automaton} (DPDA) is a quintuple~$⟨Γ,Q,q₀,qₙ,δ⟩$
  where~$Γ$ is a finite set of \emph{stack elements},~$Q$ is a finite set of
    \emph{states},~$q₀∈Q$ is the initial state,
    and~$qₙ∈Q$ is the \emph{accepting state}.
Function~$δ$ is
  the \emph{partial state transition function}~$δ:(Γ∪❴ε❵)⨉Q⨉Σ↛Q⨉Γ^*$.
  \par\itshape
  A DPDA begins its work in the initial state with a single designated stack element residing on the stack. On course of the computation,
  the stack is never emptied.
\par
  At each step, the automaton examines:~$σ∈Σ$, the next input token, the current state~$q∈Q$, and the element~$γ∈Γ$ at the top of the stack.
  Based on the values of these, it decides how to proceed:
  \par
  If~$q=q_n$, the DPDA accepts the input and halts.
  \par
