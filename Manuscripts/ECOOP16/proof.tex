...
The simple~$k=2$,~$\ell=2$) jDPDA~$A$ defined in \cref{Table:A} will serve as our running example.
...
\begin{quote}
  \javaInput[left=-2ex,minipage,width=55ex]{proof.headers.listing}
\end{quote}
...
\end{itemize}
(All invocations of~\cc{C} must make sure that actual arguments are properly constrained;
...
\begin{quote}
  \javaInput[left=0ex,minipage,width=69ex]{proof.configuration.listing}
\end{quote}
\lstinputlisting{proof.headers.listing}
...
Classes \cc{C$γ$1}, …,\cc{J$γ$}$k$
  encode stacks whose top element is~$γ₁$,…,~$γₖ$.
They all specialize these are
all specializing~\cc{C}
In~$A$ there only two of these
