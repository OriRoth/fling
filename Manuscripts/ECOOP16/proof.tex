...
\begin{wraptable}r{33ex}
  \caption{\label{Table:A} The transition function of a jDPDA~$A$,~$Σ=❴a,b,c❵$,~$Γ=❴γ₁,γ₂❵$} where~$γ₁$ is the initial element
  \begin{tabular}{c|ll}
	& \hfill~$γ₁$ 			& \hfill~$γ₂$⏎
    \midrule
$a$ 	& $\textsf{push}(γ₁,γ₁,γ₂)$ 	& $\textsf{push}(γ₂,γ₂)$⏎
$b$ 	& \hfill$\bot$ 			& $\textsf{push}(\epsilon)$⏎
$c$ 	& \hfill$\bot$ 			& $\textsf{jump}(γ₁)$⏎
...
†{%
  Incidentally,
\[
L = {\sigma_{2}}\cup\left❴ σ₁^{n₁}σ₂^{2m₁} σ₁^{n₂}σ₂^{2m₂} ⋯ σ₁^{n_r}σ₂^{2m_r-1} \; | \; r>0∧∀r', 1≤r'≤r⇒\sum_{i=1}^{r'} nᵢ≥\sum_{i=1}^{r'} mᵢ\right❵
\]
...
\begin{quote}
  \javaInput[,minipage,width=54ex]{proof.headers.listing}
\end{quote}
...
\begin{quote}
  \javaInput[,minipage,width=67ex]{proof.configuration.listing}
\end{quote}
...
\begin{quote}
  \javaInput[,minipage,width=45ex]{proof.many.listing}
\end{quote}
...
\begin{wrapfigure}[14]r{45ex}
  ...
  \javaInput[,minipage,width=45ex]{proof.cases.listing}
\end{wrapfigure}
...
The code also defines the \kk{static} variable \cc{build}, the starting point
