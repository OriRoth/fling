...
Overall, there are a total of~$k\cdot(ℓ+1)$
...
\begin{description}
  ...
  In \cref{Table:A} we see that \textsf{accept} occurs when the top of the stack is~$γ_2$.
...
@Override L ¢\gobble$¢$();
  ...
  \item[$\textsf{push}(α)$]
    ...
    The challenge is constructing, from the current parameters of the type,
      the correct five-parameter instantiation of~\cc{C}.
    ...
    The task of this sidekick is simplified if some of its generic
      ...
      result.
    \par
    Cases in point are~$δ(γ₁,σ₁)=\textsf{push}(γ₁,γ₁)$, and 
    $δ(γ₁,σ₂) = \textsf{push}(γ₂,γ_2)$ of \cref{Table:A}.
