...
\begin{Definition}[Simple-Jump Single-State Realtime Deterministic Pushdown Automaton]
  ...
  \begin{itemize}
    \item At each steps a jDPDA examines~$γ$ 
	  the element at the top of the stack, 
	  and~$σ∈Σ$, the next input character and
          ...
          \begin{quote}
            ...
            \end{enumerate}
          \end{quote}
    \item If the next character is~$\$$, then the automaton may reject or accept (but nothing else),
          depending on the value of~$γ$.
  \end{itemize}
  In addition, the automaton rejects if~$δ(γ,σ) =⊥$, or if it encounters
...
\begin{description}
  \item[\textsf{reject}]
    The default return type of \cc{\$()} in \cc{C} is \cc{$\Sigma\Sigma$}, which 
    is \emph{not} a subtype of \cc{L}. Normally the result of a call chain that ends with \cc{\$()}  
    cannot be assigned to a variable of type \cc{L}. Moreover, since \cc{$\Sigma\Sigma$} is \kk{private},
