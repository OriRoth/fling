...
This is easy to do when~$|ζ|≥1$, in which
  case the top of the stack must is known to be~$γ'$,~$γ'$ being the first
...
The definition of a configuration thus must be changed to include the \emph{two} top
  most stack elements.
But, even this would fail if the next configuration is also an~$ε$-configuration.
...
To do so, define the following directed labeled graph:
...
Each such edge takes the label~$1-|ζ|$ to denote the fact
