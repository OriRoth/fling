...
\begin{wrapfigure}[7]{r}{31ex}
  \caption{\label{Figure:id}%
  Function \cc{id()}  
  }
  \javaInput[left=0ex,width=31ex]{id.listing}
\end{wrapfigure}

Consider first \Cref{Figure:id}, showing a recipe, based on genericity for 
  creating a function whose return type 
  is the same as the receiver type, for an unbounded number of types. 
...
\begin{figure}[htb]
  \caption{\label{Figure:stack-encoding}%
    Type encoding of an unbounded stack data strucutre.
  }
  \begin{tabular}{cc}
    \parbox[c]{0.3\linewidth}{\input ../Figures/stack-classification.tikz} &
...
The type of the parameter (\kk{extends} \cc{Whales}) to a genric (\cc{Heap}) may 
