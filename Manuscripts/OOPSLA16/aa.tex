...
fluflu\urlref{https://github.com/verhas/fluflu}, a software artifact that uses
...
\begin{figure}
    \javaInput[minipage,left=-2ex]{fluflu.example.listing} 
\end{figure}
  
The automaton is single stated, the state
being both initial and accepting, with a single self transition, label with the
terminal symbol~$a$.  The almost trivial language realized by the automaton is
