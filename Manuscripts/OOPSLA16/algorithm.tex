...
Fortunately, the partial map we need to push to~$S₁$ upon
  pushing some item to~$S₀$ depends only on the identity of the latter.

Let~$Ψ$,~$Ψ: I→(Σ↛J)$ be the function returning for 
  an item $i\in I$ the dictionary that needs to be pushed into~$S₁$
  whenever~$i$ is pushed to~$S₀$.
Function $Ψ$ can be realized statically because it does not require any runtime
...
\begin{algorithm}
  ...
  an item~$i∈I$}
  \LET{$[A::=α·Y₀…Yₘ]$}{$i$} \COMMENT{break~$i$ into its components}
  \LET{$f$}{$∅$} \COMMENT{initialize output partial function}
  \FOR[for all symbols in suffix of~$i$]{$j$ in~$0…n$}
    \IF[continue to build~$f$?]{not~$\Function Nullable(Y₁…Yⱼ)$}
      \BREAK
    \FI
    \FOR[symbols that might cause jump in~$Y₀$]{$t∈\Function First(Yⱼ)$}
      \IF[is this the first update for~$t$?]{$f(t)=⊥$}
      \STATE{$f(t) = [A::=αY₀…·Yⱼ…Yₙ]$} \COMMENT{update~$f(t)$}
      \FI
    \ENDFOR
  \ENDFOR
  \RETURN{$f$}
...
The input of the algorithm is an item~$i$ ; let~$Y₀…Yₙ$ be the suffix denoted
