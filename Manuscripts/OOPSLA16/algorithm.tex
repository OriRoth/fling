...
\begin{algorithm}
  ...
  \LET{$[A::=α·Y₀…Yₙ]$}{$i$} \COMMENT{break~$i$ into its components}
  \LET{$d$}{$∅$} \COMMENT{initialize return variable}
  \FOR[for all symbols in suffix of~$i$]{$j$ in~$0…n$}
    \IF[continue to build~$d$?]{not~$\Function Nullable(Y₁…Yⱼ)$}
      \BREAK
    \FI
    \FOR[symbols that might cause jump in~$Y₀$]{$t∈\Function First(Yⱼ)$}
      \IF[is this the first update for~$t$?]{$d(t)=⊥$}
      \STATE{$d(t) = [A::=αY₀…·Yⱼ…Yₙ]$} \COMMENT{update~$f(t)$}
      \FI
    \ENDFOR
  \ENDFOR
  \RETURN{$f$}
...
encounters terminal~$t∈Σ∪❴\$❵$ on the input.
  ...
  \begin{algorithmic}
    ...
    \LET{$L$}{$∅$} \COMMENT{initialize output stack}
...
\begin{algorithm}
  ...
  \begin{algorithmic}
    ...
    \LET{$[A::=Y₀…Y_{i-1}·Yᵢ…Yₘ]$}{$i$} \COMMENT{decompose~$i$}
      ...
      \FOR[calculate entry~$\Function Δ(i,t)$]{$t∈Σ$}
        \IF[$t$ is consumed while parsing~$i$]{$t∈\Function First(Yᵢ…Yₘ)$}
          \LET{$l$}{$\Function Consolidate(i,t)$} \COMMENT{use closure table}
          \LET{$\Function Δ(i,t)$}{$\Function Push(l)$} \COMMENT{a push operation}
          \ELSIF[$t$ is consumed after parsing~$i$]{$t∈\Function Follow(A)$}
          \LET{$\Function Δ(i,t)$}{$\Function Jump(t)$} \COMMENT{a jump operation}
        \FI
      \ENDFOR
    \ENDFOR
  \end{algorithmic}
