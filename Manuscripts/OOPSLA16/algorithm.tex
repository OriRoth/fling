...
Since the item at the top of the stack was fully parsed, and since 
 \cc{begin} is in $\Function Follow(\<Constant>)$, the \RLLp should perform a
 jump operation.
...
but, now comes the~$k'$ problem: we need to continue until we ``consume'' \cc{begin},
...
\begin{algorithm}
  ...
  \IF[continue to build~$d$?]{not~$\Function Nullable(Y₂…Y_{j-1})$}
    ...
    \IF[\nth{1} update of key~$t$ in dictionary~$d$?]{$d(t)=⊥$}
      ...
      %\STATE{$d(t) = Consolidate([A::=αY₁…·Yⱼ…Yₙ],t)$} \COMMENT{update~$d(t)$}
      \FI
    \ENDFOR % Symbols
  \ENDFOR
  \RETURN{$d$}
