...
As the last step before constructing the action table, we
  define the function~$\textsf{JS}: Q⨉Σ→(ℕ↛Q)$ (jump set).
...
\begin{algorithm}[h]
  \begin{algorithmic}
    ...
    \STATE{$f←❴❵$}
    \FOR{$[ A→α·aβ, b,ℓ]∈\function NonKernel(q)$} % Problem: we add to the function only for non-kernel items, now the distinction is a bit elusive.
      \STATE{$ f(ℓ) = \function Goto(\function Goto(q,A),b)$}
    \ENDFOR
    \RETURN{$f$}
  \end{algorithmic}
\end{algorithm}
...
\begin{itemize}
  ...
  \item If~$[ A→α·β,σ,ℓ]∈q~$ and~$σ'∈\function First(β)$ 
    then~$\table Action[q,σ'] = \function shift(\function JS(q,σ'),\function
    Goto(q,a))$
...
\begin{Definition}[Equivalence relation]
  An LR state~$q_{LR}$ and a JLR state~$q_{JLR}$ are said to be equivalent
  under~$\cong$ if for each item~$i \in q_{LR}$ there exists an item~$j \in
  q_{JLR}$ such that~$i = \function Clean(j)$. 
\end{Definition}
