...
In the case~$R=X::=Y$,~$Y$ % MAYBE WE SHOULD CHANGE THIS TO AN EXAMPLE WITH PARTIALLY NULLABLE PREFIX
...
\begin{figure}
  ...
  \begin{Grammar}
    \begin{aligned}
      \<Program> & ::= \cc{program} \cc{id} \<Parameters>\~\cc{;} \<Definitions>⏎
                 &\<Body>\hfill⏎
      ...
      \<Procedure> & ::= \cc{procedure}\cc{id}\<Parameters>\~\cc{;}\<Definitions>⏎
                    & \<Body> \hfill⏎
...
Thus, all derivations of \<Procedure> must begin with terminal
  ...
  containts only~\cc{procedure}, as can be seen
...
%\subsubsection{Examples for~\texorpdfstring{$k'$}{k'} for the grammar in~\cref{figure:running}}
...
The problem is that every such operation is an~$ε$-move of the
  parser's DPDA, and as was shown in~\urlref{http://stlevy.cswp.cs.technion.ac.il/wp-content/uploads/sites/50/2015/12/00.pdf} it causes
...
\subsection{SLL: the Simplified LL parser}
\label{section:sll}
Building on our intuition, we now present the \emph{SLL} parser, a variant of
the traditional LL parser~\cite{lewis:66}, but more economic in its use of the
stack resource, and perhaps of independent interest.
