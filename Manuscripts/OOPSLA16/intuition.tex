...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
      ...
      \IF[ancipate to match terminal~$X$]{$X∈Σ$}
      ...
      \ELSE[anticipated~$X$ must be a nonterminal]
        \LET{$R$}{$\Function δ(X, \Function peek())$}\COMMENT{to which rule~$X$ is parsed?}
        \IF[no rule found]{$R=⊥$}
          \STATE{\textsc{Reject}}\COMMENT{automaton halts in error}
...
\begin{figure}
  ...
  \begin{Grammar}
    \begin{aligned}
      \<Program> & \Derives \cc{program} \cc{id} \<Parameters>\~\cc{;} \<Definitions>⏎
                 &\<Body>\hfill⏎
