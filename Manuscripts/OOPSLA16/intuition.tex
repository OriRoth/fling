        ...
        \FOR[for every~$X$-production]{$X::=Y₀…Yₖ∈P$}
          \FOR[for each symbol in the RHS]{$i∈❴0…k❵$}
            \IF[if the prefix is nullable]{$\Function Nullable(Y₀…Y_{i-1})$}
              ...
              \COMMENT{add~$\Function First(Yᵢ)$ to~$X$}
            \ENDIF
          \ENDFOR
        \ENDFOR
      \FI
    ...
    \REPEAT[until no~$\Function Follow(·)$ set have changed]
      \FOR[for each grammar rule]{$A::=Y₀…Yₖ∈\Function get(P)$}
        \FOR[for each symbol in the RHS]{$i∈❴0…k❵$}
          ...
          \IF[if suffix not nullable]{not~$\Function Nullable(Y_{i+1}…Yₖ)$}
            \CONTINUE
          \ENDIF
            \STATE{$\Function Follow(Yᵢ) \, ∪= \Function First(A)$}
            \COMMENT{add~$\Function Follow(A)$ to~$\Function Follow(Yᵢ)$}
        \ENDFOR
      \ENDFOR
    \UNTIL{No changes in any~$\Function First(·)$ set}
  \end{algorithmic}
...
The LL(1) parser is based on the parser's prediction table, with
  for a given top of of the stack nonterminal and input symbol,
  provides the next rule to be parsed.
The parser generator basically fills this table, leaving the
  rest to the parsing algorithm (depicted in~\cref{figure:ll-parser}
