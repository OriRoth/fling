...
\begin{algorithm}[p]
  ...
  Function~$\Function Goto(I,X)$: generate successive item set for~$I$ after seeing~$X$}
  \begin{algorithmic}
    \INPUT{a set of JLR items~$I$}
...
A JLR is like the jDPDA~$A=⟨Γ,q₀,δ⟩$ defined in~\cref{definition:JDPDA},
  ...
  % Red rule , only on non-kernel items
   ...
   \item~$δ(q,σ)= q \: f \: p~$ if~$η_{LR}(q,σ)=\Function Reduce(A→ε)$
     and~$δ_{LR}(q,A)=Goto(q')$ and~$η_{LR}(q',σ)=\Function Shift(p)$
     and we advanced only on~$I⊆\textsf{NonKernel}_q$
     items, then
     \[
      f = ❴σ'↦q' \; | \;∃i∈I. lookahead(i)=σ' \,∧\, η_{LR}(q,σ')=\Function Shift(q')❵
     \]
