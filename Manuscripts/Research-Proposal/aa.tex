...
An object of type toilete seat is created in the \cc{down} state, but it can then be \cc{raise}d to the \cc{up} state, and 
...
``open'' or ``closed''. Ivoking a \cc{read()} method on the object is only permitted when the file is an ``open'' state.
