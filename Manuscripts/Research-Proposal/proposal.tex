...
\begin{equation}\label{eq:element}
  \begin{aligned}
  \<configuration-element> ::= & \<configuration-comment> \hfill⏎
 57                               |\ & \<rule> \hfill⏎
 58                               |\ & \<declaration> \hfill
 59   \end{aligned}
 60   .
 61 \end{equation}
...
Accordingly, the designer of a fluent API has to follow this three conceptual steps.
first identifies the \emph{vocabulary}, i.e.,
...
then, there are three method calls, and the vocabulary has three items in it.
    \begin{itemize}
      \item \cc{allowing (any(Object.class))}
  ¢¢.method("get.*")
  ¢¢.withNoArguments();
\end{enumerate}
