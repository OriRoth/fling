 ...
 * @author Ori Roth
 */
public class BNF extends SimpleBNF {
  ...
  public BNF(final Set<Verb> Σ, final Set<Variable> Γ, final Set<DerivationRule> R, final Variable startVariable,
  ...
  public BNF reachableSubBNF() {
    ...
    while (previousCount < subV.size()) {
      ...
      for (final DerivationRule rule : R) {
        if (!newSubV.contains(rule.lhs))
          continue;
        subR.add(rule);
        for (final ExtendedSententialForm sf : rule.rhs) for (final GeneralizedSymbol symbol : sf) if (symbol.isVerb())
    ...
    return new BNF(subΣ, subV, subR, ε, null, null, null, true);
  }
  ...
  private Map<GeneralizedSymbol, Set<Verb>> getFirsts() {
    ...
    for (boolean changed = true; changed;) {
      changed = false;
      for (final Variable v : Γ) for (final ExtendedSententialForm sf : rhs(v)) for (final GeneralizedSymbol symbol : sf) {
  ...
  private Map<Variable, Set<Verb>> getFollows() {
    ...
    for (boolean changed = true; changed;) {
      changed = false;
      for (final Variable v : Γ) for (final ExtendedSententialForm sf : rhs(v)) for (int i = 0; i < sf.size(); ++i) {
  ...
  public static BNF toBNF(final PlainBNF specification) {
    ...
    for (final Rule rule : specification.rule) if (rule instanceof Derivation) {
      // Derivation rule.
      ...
      if (derivation.ruleBody instanceof ConcreteDerivation) {
        ...
        for (RuleTail tail : concrete.ruleTail) if (tail instanceof ConcreteDerivationTail)
          // Concrete tail.
  ...
  @Deprecated private static class Builder {
    ...
    public class Derive {
      ...
      public Builder to(final GeneralizedSymbol... sententialForm) {
        final ExtendedSententialForm processedSententialForm = new ExtendedSententialForm(Arrays.stream(sententialForm) //
            .map(symbol -> {
  ...
  private static Optional<GeneralizedSymbol> extractANTLRSentential(Builder $, Object element, Counter nameCounter) {
    if (element instanceof List) {
      ...
      for (Object item : elements) extractANTLRSentential($, item, nameCounter).ifPresent(items::add);
    ...
    if (element instanceof BlockAST) {
      ...
      Variable top = Variable.byName(intermediateVariableName + nameCounter.getAndInc());
      List<GeneralizedSymbol> items = new ArrayList<>();
      for (Object item : block.getChildren()) extractANTLRSentential($, item, nameCounter).ifPresent(items::add);
    ...
    if (element instanceof StarBlockAST) {
      Optional<GeneralizedSymbol> inner = extractANTLRSentential($, ((StarBlockAST) element).getChildren(), nameCounter);
      return inner.map(GeneralizedSymbol::noneOrMore);
    }
    if (element instanceof PlusBlockAST) {
      Optional<GeneralizedSymbol> inner = extractANTLRSentential($, ((PlusBlockAST) element).getChildren(), nameCounter);
