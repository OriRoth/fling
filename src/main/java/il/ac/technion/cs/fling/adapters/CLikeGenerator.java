...
import il.ac.technion.cs.fling.internal.compiler.api.dom.Type.Instantiation.Plain;
import il.ac.technion.cs.fling.internal.compiler.api.dom.Type.Instantiation.Plain.Parametrized;
...
public abstract class CLikeGenerator extends APIGenerator {
  ...
  @Override public String renderInstnatiation(Type.Name name, List<Type.Instantiation> arguments) {
  ...
  @Override public final String toString(Parametrized i) {
    return toString(i.outer()) + String.format("<%s>", i.arguments().map(this::render).collect(joining(", ")));
  }
  @Override public final String toString(Plain i) {
