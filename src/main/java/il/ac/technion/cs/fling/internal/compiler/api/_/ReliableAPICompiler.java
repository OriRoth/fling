...
import il.ac.technion.cs.fling.internal.compiler.api._.Type.Name;
...
public class ReliableAPICompiler extends DPDAToModel {
  ...
  private Type.Grounded common(final δ<Named, Token, Named> δ, final Word<Named> α, final Set<Named> legalJumps,
    ...
    if (α.isEmpty()) {
      ...
      final var name = encodedName(δ.q$, δ.α, legalJumps);
      return name == null ? Type.Grounded.BOTTOM
          : Type.Grounded.of(name).with(getTypeArguments(legalJumps, isInitialType));
    }
    if (δ.α.isEmpty())
    ...
    final var name = encodedName(δ.q$, δ.α, typeArguments.keySet());
    return name == null ? Type.Grounded.BOTTOM : //
        Type.Grounded.of(encodedName(δ.q$, δ.α, typeArguments.keySet()))
