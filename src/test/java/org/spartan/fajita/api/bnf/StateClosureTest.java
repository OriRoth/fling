...
public class StateClosureTest {
  ...
  @Test public void testInitialItemKept() {
  ...
  @Test public void testClosureOnDerivationRule() {
    BNF bnf = new BNFBuilder(Term.class, NT.class) //
        .startConfig() //
        .setApiNameTo("TEST") //
        .setStartSymbols(NT.S) //
 ...
 @Test public void testClosureOnInheritenceRule() {
    ...
    DerivationRule StoA_Rule = dRules.stream().filter(r -> r.lhs.equals(NT2.S) && r.expression.get(0).equals(NT2.A)).findAny()
        .get();
    DerivationRule StoB_Rule = dRules.stream().filter(r -> r.lhs.equals(NT2.S) && r.expression.get(0).equals(NT2.B)).findAny()
        .get();
    DerivationRule StoC_Rule = dRules.stream().filter(r -> r.lhs.equals(NT2.S) && r.expression.get(0).equals(NT2.C)).findAny()
        .get();
  ...
  @Test public void testIterativeClosure() {
    BNF bnf = new BNFBuilder(Term.class, NT2.class) //
        .startConfig() //
        .setApiNameTo("TEST") //
        .setStartSymbols(NT2.S) //
    ...
    Set<DerivationRule> dRules = new HashSet<>(bnf.getDerivationRules());
    bnf.getInheritenceRules().forEach(iRule -> dRules.addAll(iRule.getAsDerivationRules()));
    DerivationRule AtoB_Rule = dRules.stream().filter(r -> r.lhs.equals(NT2.A) && r.expression.get(0).equals(NT2.B)).findAny()
        .get();
    DerivationRule AtoC_Rule = dRules.stream().filter(r -> r.lhs.equals(NT2.A) && r.expression.get(0).equals(NT2.C)).findAny()
        .get();
  ...
  @Test public void testSameItemAppearsOnlyOnce() {
    BNF bnf = new BNFBuilder(Term.class, NT.class) //
        .startConfig() //
        .setApiNameTo("TEST") //
        .setStartSymbols(NT.S) //
  ...
  @Test public void testClosureWithMultipleStartVariables() {
    BNF bnf = new BNFBuilder(Term.class, NT2.class) //
        .startConfig() //
        .setApiNameTo("TEST") //
    ...
    Set<DerivationRule> dRules = new HashSet<>(bnf.getDerivationRules());
    bnf.getInheritenceRules().forEach(iRule -> dRules.addAll(iRule.getAsDerivationRules()));
    DerivationRule AtoA_Rule = dRules.stream().filter(r -> r.lhs.equals(NT2.A) && r.expression.get(0).equals(NT2.A)).findAny()
        .get();
    DerivationRule AtoC_Rule = dRules.stream().filter(r -> r.lhs.equals(NT2.A) && r.expression.get(0).equals(NT2.C)).findAny()
        .get();
    DerivationRule S_Rule = dRules.stream().filter(r -> r.lhs.equals(NT2.S)).findAny().get();
