...
public class ItemClosureTest {
    ...
    @Test
    public void testInitialItemKept() throws ReduceReduceConflictException, ShiftReduceConflictException {
    ...
    @Test
    public void testClosureOnDerivationRule() throws ReduceReduceConflictException, ShiftReduceConflictException {
	BNF bnf = new BNFBuilder(Term.class, NT.class) //
		.startConfig() //
		.setApiNameTo("TEST") //
		.setStartSymbols(NT.S) //
    ...
    @Test
    public void testClosureOnInheritenceRule() throws ReduceReduceConflictException, ShiftReduceConflictException {
	...
	DerivationRule StoA_Rule = dRules.stream()
		.filter(r -> r.lhs.equals(NT2.S) && r.getChildren().get(0).equals(NT2.A)).findAny().get();
	DerivationRule StoB_Rule = dRules.stream()
		.filter(r -> r.lhs.equals(NT2.S) && r.getChildren().get(0).equals(NT2.B)).findAny().get();
	DerivationRule StoC_Rule = dRules.stream()
		.filter(r -> r.lhs.equals(NT2.S) && r.getChildren().get(0).equals(NT2.C)).findAny().get();
	...
	return bnf.getInheritenceRules().stream().filter(r -> r.lhs.equals(bnf.getAugmentedStartSymbol())).findAny()
		.get().getAsDerivationRules().iterator().next();
    }

    @Test
    public void testIterativeClosure() throws ReduceReduceConflictException, ShiftReduceConflictException {
	BNF bnf = new BNFBuilder(Term.class, NT2.class) //
		.startConfig() //
		.setApiNameTo("TEST") //
		.setStartSymbols(NT2.S) //
	...
	Set<DerivationRule> dRules = new HashSet<>(bnf.getDerivationRules());
	bnf.getInheritenceRules().forEach(iRule -> dRules.addAll(iRule.getAsDerivationRules()));
	DerivationRule AtoB_Rule = dRules.stream()
		.filter(r -> r.lhs.equals(NT2.A) && r.getChildren().get(0).equals(NT2.B)).findAny().get();
	DerivationRule AtoC_Rule = dRules.stream()
		.filter(r -> r.lhs.equals(NT2.A) && r.getChildren().get(0).equals(NT2.C)).findAny().get();
	...
	Iterator<DerivationRule> augmentedRulesIterator = bnf.getInheritenceRules().stream()
		.filter(r -> r.lhs.equals(bnf.getAugmentedStartSymbol())).findAny().get().getAsDerivationRules()
		.iterator();
